...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self._setup_model()
        self.optim = get_optimizer(optim_aliase, [{'params': self.qf_params_group}], lr)
    ...
    def _setup_model(self):
        ...
        self.latent_pi = nn.Sequential(*create_mlp(input_dim=self.features_dim2,
                                                   output_dim=-1,
        ...
        qf_net_group = [self.latent_pi.parameters(), self.mu.parameters(), self.log_std.parameters()]
        self.qf_params_group = [params for net in qf_net_group for params in net]


    def _get_actions_dist_params(self,
                                obs:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        state = self.srl_model.phi1(self.feature_extractor1(obs))  # Zo
        state = state.detach()
        #get mean and std params of the actions distribution
        latent_pi = self.latent_pi(state)    # common neural networks outputs
