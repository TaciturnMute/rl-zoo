...
'''
this env is used for portfolio management which the actor and critic structure is MLP,
so env step return one time step state.
'''

class StockPortfolioEnv_new_feature(gym.Env):
    '''
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(low=-np.inf,
                                            high=np.inf,
                                            shape=(self.state_space, self.stock_dim),
                                            )
        ...
        self.last_weights = np.ones(self.stock_dim) / self.stock_dim
    ...
    def _get_state(self):
        self.state = np.append(
            self.portfolio_value_vector.reshape(1, -1) * 1e-5,  # to normalize, make asset value around 1
            self.last_weights.reshape(1, -1),
            axis=0,
        )
        self.state = np.append(
            self.state,
