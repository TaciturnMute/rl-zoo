...
'''
used for RNNs models
env could save a period of time's states and aggregate them as the input that model need.
'''

matplotlib.use("Agg")

class StockPortfolioEnv_series(gym.Env):
    '''
    ...
    def __init__(
        ...
        self.reward_aliases = ['asset_diff', 'asset_diff_dji', 'sharpe_rario_diff']
        self.day = day
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        if not self.terminal:
            ...
            last_day_memory = self.data  # vt-1
            ...
            price_pct_vector = (self.data.close.values / last_day_memory.close.values)  # vt/vt-1
            portfolio_return = sum((price_pct_vector - 1) * weights)
            # new_portfolio_value = self.portfolio_value * (1 + portfolio_return)
            self.portfolio_value_vector = self.portfolio_value_vector * price_pct_vector
            self.portfolio_value = sum(self.portfolio_value_vector * price_pct_vector)
            ...
            self.returns.insert(0,"date", np.array(self.df.date.unique()))
            self.returns.dropna()
